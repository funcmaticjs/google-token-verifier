...
const GoogleTokenVerifier = require('../lib/verifier')
...
const VALID_TOKEN = process.env['TEST_VALID_GOOGLE_TOKEN']

describe('Token Verification', () => {
  let verifier = null
  beforeEach(async () => {
    verifier = new GoogleTokenVerifier({ GOOGLE_JWKS_URI: process.env.GOOGLE_JWKS_URI })
  })  
  ...
  it ('should throw if setup with incorrect JWKS URI', async () => {
    let badverifier = new GoogleTokenVerifier({ GOOGLE_JWKS_URI: "https://www.googleapis.com/bad/jwks/uri" })
    let error = null
    try {
